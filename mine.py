...
def mine_block(new_block, rounds=STANDARD_ROUNDS, start_nonce=0):
    print
    "Mining for block %s. start_nonce: %s, rounds: %s" % (new_block.index, start_nonce, rounds)
    ...
    for nonce in nonce_range:
        ...
        if str(new_block.hash[0:NUM_ZEROS]) == '0' * NUM_ZEROS:
            print
            "block %s mined. Nonce: %s" % (new_block.index, new_block.nonce)
